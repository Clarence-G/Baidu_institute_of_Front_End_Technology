# structure: conv1 conv2 pool fc1 fc2
import numpy  as np
...
with tf.Session() as sess:
    ...
    sess.run(init_op)
    train_writer = tf.summary.FileWriter('train2', sess.graph)
    for i in range(training_steps):
        xs,ys = sess.run(next_element)
        #xs = xs.eval()
        #ys = ys.eval()
        xs = np.reshape(xs,(8,48,64,1))
        ...
        saver.save(sess, "train2/savesample.ckpt", global_step=global_step)
