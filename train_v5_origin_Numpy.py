...
def inference(input_tensor, train, regularizer):
    ...
    pool_shape = input_tensor.get_shape().as_list()
    nodes = pool_shape[1] * pool_shape[2] * pool_shape[3]
    reshaped = tf.reshape(input_tensor, [pool_shape[0], nodes])
...
with tf.Session() as sess:
    ...
    train_writer = tf.summary.FileWriter('train5', sess.graph)
    for i in range(training_steps):
        ...
        if i % 2 == 0:
            ...
            saver.save(sess, "train5/savesample.ckpt", global_step=global_step)
